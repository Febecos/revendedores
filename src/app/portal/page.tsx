...
export default function Portal() {
  ...
  // Toggle precio público / mayorista
  ...
  async function verificarToken(t: string) {
    try {
      ...
      if (!data || data.length === 0) { setError('token_invalido'); setLoading(false); return }
      setRev(data[0])
    } catch {
      setError('error_red')
    } finally {
  ...
  async function buscarBomba() {
    ...
    setErrCalc(null)
    try {
      const url = `${API_BOMBAS}?height=${altura}&liters=${litros}&diameter=${diametro}&season=verano`
      const res = await fetch(url)
      const data = await res.json()
      if (!data.ok) { setErrCalc(data.error || 'No se encontró bomba'); return }
      setResultado(data)
    } catch {
  ...
  if (error || !rev) return (
    <Pantalla emoji="⚠️" titulo="Error de conexión"
      sub="No pudimos verificar tu acceso. Intentá recargar la página."
    />
  )
      ...
      <div style={s.header}>
        <div style={s.headerInner}>
          ...
          <div style={s.headerRight}>
            ...
            <div style={s.descuentoBadge}>
              {rev.descuento_pct}% OFF
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={s.content}>
        ...
        <div style={s.toggleWrap}>
          ...
          <div style={s.toggleBtns}>
            <button
              onClick={() => setMostrarPublico(false)}
              style={{ ...s.toggleBtn, ...(mostrarPublico ? {} : s.toggleBtnActive) }}
            >
              Mayorista ({rev.descuento_pct}% OFF)
            </button>
            <button
              onClick={() => setMostrarPublico(true)}
              style={{ ...s.toggleBtn, ...(mostrarPublico ? s.toggleBtnActive : {}) }}
            >
              Precio público
            </button>
        ...
        <div style={s.infoGrid}>
          ...
          <div style={s.infoCard}>
            ...
            <div style={s.infoSub}>Los precios y disponibilidad se actualizan automáticamente</div>
          </div>
          <div style={s.infoCard}>
            <div style={s.infoEmoji}>🤝</div>
          ...
          <div style={s.infoCard}>
            <div style={s.infoEmoji}>🔗</div>
            <div style={s.infoTitulo}>Tu link de acceso</div>
            <div style={{ fontFamily: 'monospace', fontSize: 10, color: '#7a9ab5', wordBreak: 'break-all', marginTop: 4 }}>
